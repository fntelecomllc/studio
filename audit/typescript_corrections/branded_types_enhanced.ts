 ...
 */
export function safeBigIntReplacer(key: string, value: any): any {
 ...
 */
export function parseJSONWithBigInt(text: string, int64Fields: Set<string>): any {
 ...
 */
export function transformInt64Fields<T extends Record<string, any>>(
  obj: T,
  int64Fields: (keyof T)[]
): T {
  const result = { ...obj };
  ...
  for (const field of int64Fields) {
    ...
    if (value !== null && value !== undefined) {
      result[field] = createSafeBigInt(value) as any;
    }
  }
 ...
 */
export function prepareForSerialization<T extends Record<string, any>>(
  obj: T,
